...
document.addEventListener("DOMContentLoaded", () => {
        ...
        <div class="prst-form mb-4">
          ...
          <div class="row">
            <div class="col-md-3 mb-3">
              <label class="form-label">Caja de empalme</label>
              <input type="number" class="form-control prst-caja-empaque" min="0">
            </div>
            <div class="col-md-3 mb-3">
              <label class="form-label">Reserva</label>
              <input type="number" class="form-control prst-reserva" min="0">
            </div>
            <div class="col-md-3 mb-3">
              <label class="form-label">NAP</label>
              <input type="number" class="form-control prst-nap" min="0">
            </div>
            <div class="col-md-3 mb-3">
              <label class="form-label">SPT</label>
              <input type="number" class="form-control prst-spt" min="0">
            </div>
          </div>
          
          <div class="row">
            <div class="col-md-6 mb-3">
              <label class="form-label">Bajante</label>
              <input type="number" class="form-control prst-bajante" min="0">
            </div>
          </div>
  ...
  function saveCenso() {
    ...
    if (!elementoNA.checked && elementosCheckboxes.length === 0) {
      document.getElementById("elementosCheckboxes").classList.add("is-invalid")
      const feedback = document.createElement("div")
      feedback.className = "invalid-feedback d-block"
      feedback.textContent = "Seleccione N/A o al menos un elemento"
      document.getElementById("elementosCheckboxes").after(feedback)
      isValid = false
    }
  ...
  function validateCensoForm() {
    ...
    // Validar elementos existentes (al menos N/A o algún elemento)
    const elementoNA = document.getElementById("elementoNA")
    const elementosCheckboxes = document.querySelectorAll("#elementosCheckboxes .elemento-checkbox:checked")
    ...
    if (!elementoNA.checked && elementosCheckboxes.length === 0) {
      ...
      document.getElementById("elementosCheckboxes").classList.remove("is-invalid")
    }
    ...
    fotoFields.forEach((field) => {
      ...
      if (preview.src.includes("placeholder-image.png")) {
    ...
    if (cantidadPRST > 0) {
      const prstForms = document.querySelectorAll(".prst-form")
      if (prstForms.length !== cantidadPRST) {
        isValid = false
      } else {
        prstForms.forEach((form, index) => {
          const nombre = form.querySelector(".prst-nombre")
          const cables = form.querySelector(".prst-cantidad-cables")

          if (!nombre.value || !cables.value) {
            if (!nombre.value) nombre.classList.add("is-invalid")
            if (!cables.value) cables.classList.add("is-invalid")
            isValid = false
          } else {
  ...
  // Función auxiliar para obtener datos de PRST
  function getPRSTData() {
    ...
    prstForms.forEach((form) => {
      prstData.push({
        ...
        cajaEmpalme: form.querySelector(".prst-caja-empaque")?.value || "0",
        reserva: form.querySelector(".prst-reserva")?.value || "0",
        nap: form.querySelector(".prst-nap")?.value || "0",
        spt: form.querySelector(".prst-spt")?.value || "0",
        bajante: form.querySelector(".prst-bajante")?.value || "0",
  ...
  function getSelectedElementos2() {
    const elementos = []
    const elementoNA = document.getElementById("elementoNA")
