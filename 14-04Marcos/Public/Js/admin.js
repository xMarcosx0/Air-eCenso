document.addEventListener("DOMContentLoaded", () => {
  ...
  function cargarUsuarios() {
    ...
    usuariosFiltrados.forEach((usuario) => {
      ...
      const estadoActivo = usuario.activo !== false // Si no está definido, se considera activo
      const estadoBadge = estadoActivo ? "bg-success" : "bg-danger"
    ...
    <tr>
      ...
      <td>
        <button class="btn btn-sm btn-info me-1" onclick="verDetalleUsuario('${usuario.id}')">
          <i class="bi bi-eye"></i>
        </button>
  ...
  function abrirModalNuevoUsuario() {
    ...
    // Ocultar campos de nombre de brigada y cargo
    const nombreBrigadaField = document.getElementById("nombreBrigada").closest(".mb-3")
    const cargoField = document.getElementById("cargo").closest(".mb-3")

    if (nombreBrigadaField) nombreBrigadaField.style.display = "none"
    if (cargoField) cargoField.style.display = "none"
  ...
  function guardarUsuario() {
    ...
    if (userId) {
      ...
      // Crear nuevo usuario
      if (!password) {
        mostrarAlerta("Por favor, ingrese una contraseña para el nuevo usuario.", "error")
        document.getElementById("loadingIndicator").style.display = "none"
        return
      }
        ...
        .then((userCredential) => {
          ...
          setDoc(doc(db, "usuarios", user.uid), userData)
            ...
            .catch((error) => {
              ...
              mostrarAlerta("Error al guardar datos del usuario: " + error.message, "error")
            })
        })
        .catch((error) => {
          document.getElementById("loadingIndicator").style.display = "none"
          ...
          mostrarAlerta("Error al crear usuario: " + errorMessage, "error")
        })
    }
  }
  ...
  window.editarUsuario = (id) => {
    ...
    // Mostrar/ocultar campos específicos según el rol
    ...
    // Añadir campo de estado (activo/inactivo)
    const estadoHtml = `
    <div class="mb-3">
      <label for="estado" class="form-label">Estado</label>
      <select class="form-select" id="estado">
        <option value="activo" ${usuario.activo !== false ? "selected" : ""}>Activo</option>
        <option value="inactivo" ${usuario.activo === false ? "selected" : ""}>Inactivo</option>
      </select>
    </div>
  `

    // Verificar si ya existe el campo de estado
    if (!document.getElementById("estado")) {
      // Insertar antes del botón de guardar
      const modalFooter = document.querySelector("#modalUsuario .modal-footer")
      if (modalFooter) {
        modalFooter.insertAdjacentHTML("beforebegin", estadoHtml)
      }
    } else {
      // Actualizar el valor del campo existente
      document.getElementById("estado").value = usuario.activo !== false ? "activo" : "inactivo"
    }
  ...
  window.verDetalleUsuario = (id) => {
    ...
    document.getElementById("detalleUsuarioId").textContent = usuario.id
    document.getElementById("detalleUsuarioNombre").textContent = `${usuario.nombre} ${usuario.apellido || ""}`
    document.getElementById("detalleUsuarioUsername").textContent = usuario.usuario
    document.getElementById("detalleUsuarioCorreo").textContent = usuario.correo
    ...
    document.getElementById("detalleUsuarioRol").textContent = rolTexto

    // Mostrar estado
    const estadoActivo = usuario.activo !== false
    document.getElementById("detalleUsuarioEstado").textContent = estadoActivo ? "Activo" : "Inactivo"
    document.getElementById("detalleUsuarioEstado").className = estadoActivo ? "text-success" : "text-danger"

    // Mostrar campos específicos según el rol
    const filaBrigada = document.getElementById("filaBrigada")
    const filaSector = document.getElementById("filaSector")
    const filaPRST = document.getElementById("filaPRST")

    filaBrigada.style.display = "none"
    filaSector.style.display = "none"
    filaPRST.style.display = "none"

    if (usuario.rol === "brigada") {
      filaBrigada.style.display = ""
      filaSector.style.display = ""
      document.getElementById("detalleUsuarioBrigada").textContent = usuario.nombreBrigada || "No especificado"
      document.getElementById("detalleUsuarioSector").textContent = usuario.sector || "No especificado"
    } else if (usuario.rol === "prst") {
      filaPRST.style.display = ""
      document.getElementById("detalleUsuarioPRST").textContent = usuario.nombrePRST || "No especificado"
    }

    // Mostrar modal
    const modalDetalleUsuario = new bootstrap.Modal(document.getElementById("modalDetalleUsuario"))
    modalDetalleUsuario.show()
  }
  ...
  const { getFirestore, doc, setDoc, updateDoc } = window.firebase.firestore
